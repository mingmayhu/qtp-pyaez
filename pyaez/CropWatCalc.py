...
class CropWatCalc(object):
    ...
    def setClimateData(self, pet, precipitation, wind_speed, min_temp, max_temp, active_layer_thickness, soil_moisture):
        self.peto = pet
        self.Prec = precipitation
        self.wind_sp = wind_speed
        self.min_temp = min_temp
        self.max_temp = max_temp
        self.alt = active_layer_thickness
    ...
    def calculateMoistureLimitedYieldNew(self):
        ...
        for ii in range(self.cycle_len-1):
            eff_D = min(D[ii], self.alt[ii])

            if self.Prec[ii] >= petc_stage[ii]:
                peta_stage[ii] = petc_stage[ii]
            elif self.Prec[ii] + self.soil_moisture[ii] >= self.Sa*eff_D*(1-self.pc):
                peta_stage[ii] = petc_stage[ii]
            else:
                kk = (self.soil_moisture[ii]+self.Prec[ii]) / (self.Sa*eff_D*(1-self.pc))
                peta_stage[ii] = kk * petc_stage[ii]
